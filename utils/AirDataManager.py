...
class AirDataManager:
    ...
    def get_random_data(self, n_samples=10):
        data_names = self.metadataOperator.sampleData(n=n_samples)
        hr_images = self.read_hr_images(data_names)
        lr_images = self.read_lr_images(data_names)
        return hr_images, lr_images

    def read_hr_images(self, data_points):
        images = []
        for data_point in data_points:
            directory = self.data_folder + "\\" + self.hr_dataset_name + "\\" + data_point + ".png.png"
    ...
    def read_lr_images(self, data_points):
        images = []
        for data_point in data_points:
            directory = self.data_folder + "\\" + self.lr_dataset_name + "\\" + data_point + ".png.png"
            images.append(Image.open(directory))
        images_tensor = torch.stack([transforms.ToTensor()(img) for img in images])
        images_tensor = images_tensor.permute(0, 2, 3, 1)
        return images_tensor
